...
3. 可選擇設定 OpenRouter / NVIDIA 作為 @bot 問答備援：
   ...
   - `AI_FALLBACK_PROVIDERS=gemini,nvidia`
...
| `ARCHIVE_AI_MODE` | 歸檔分類/摘要/討論串是否用 AI，預設 `local`；可設 `mentions` 或 `all` |
| `AI_PROVIDER` | @bot 問答主要 provider，預設 `openrouter` |
| `AI_FALLBACK_PROVIDERS` | @bot 問答備援 provider，預設 `gemini,nvidia` |
| `OPENROUTER_API_KEY` | OpenRouter API key，用於 @bot 問答主要回答 |
| `OPENROUTER_MODEL` | OpenRouter 模型，預設 `openrouter/auto` |
| `NVIDIA_API_KEY` | NVIDIA NIM API key，用於最後備援回答 |
| `NVIDIA_MODEL` | NVIDIA NIM 模型，預設 `meta/llama-3.3-70b-instruct` |
